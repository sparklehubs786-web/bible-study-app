...
let currentTeacher=null,currentStudentId=null,selectedGrade=null,gradeStyle='letter',currentSubmissionId=null,allStudents=[],allSubmissions=[];
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  const authTimeout=setTimeout(()=>{console.warn('Auth timeout — trying localStorage');tryLocalStorageFallback();},15000);
  ...
  auth.onAuthStateChanged(async user=>{
      ...
      try{
        ...
        if(parsed.email===user.email){
          if(parsed.status==='pending_payment'){
            showLoading(false);
            window.location.href='pricing.html?plan='+(parsed.plan||'medium')+'&email='+encodeURIComponent(user.email)+'&name='+encodeURIComponent(parsed.name||'');
            return;
          }
          currentTeacher=parsed;currentTeacher.uid=user.uid;currentTeacher.email=user.email;
          showLoading(false);
          await showDashboard();
    ...
    // STEP 2: Firestore
    try{
      const snap=await Promise.race([db.collection('users').doc(user.uid).get(),new Promise((_,r)=>setTimeout(()=>r(new Error('timeout')),8000))]);
      if(!snap.exists){
        // No profile — create minimal
        currentTeacher={uid:user.uid,name:user.displayName||user.email.split('@')[0],email:user.email,classCode:genCode(),plan:'medium',maxStudents:30,status:'active',expiresAt:Date.now()+(90*24*60*60*1000)};
        localStorage.setItem('dtwd_teacher',JSON.stringify(currentTeacher));
        showLoading(false);await showDashboard();return;
      }
      const data=snap.data();
      if(data.role==='student'){showLoading(false);window.location.replace('student.html');return;}
      if(data.status==='pending_payment'){
        showLoading(false);
        window.location.href='pricing.html?plan='+(data.plan||'medium')+'&email='+encodeURIComponent(user.email)+'&name='+encodeURIComponent(data.name||'');
        return;
      }
      currentTeacher={uid:user.uid,name:data.name||user.displayName||user.email.split('@')[0],email:user.email,classCode:data.classCode||genCode(),plan:data.plan||'medium',maxStudents:{small:15,medium:30,large:60,xl:100}[data.plan]||30,status:data.status||'active',expiresAt:data.expiresAt||(Date.now()+90*24*60*60*1000)};
      if(!data.classCode)db.collection('users').doc(user.uid).set({classCode:currentTeacher.classCode},{merge:true}).catch(()=>{});
      localStorage.setItem('dtwd_teacher',JSON.stringify(currentTeacher));
      showLoading(false);await showDashboard();
    }catch(fsErr){
      console.warn('Firestore error:',fsErr.message);
      if(savedTeacher){try{const p=JSON.parse(savedTeacher);if(p.status==='active'){currentTeacher=p;currentTeacher.uid=user.uid;showLoading(false);await showDashboard();return;}}catch(e){}}
      currentTeacher={uid:user.uid,name:user.displayName||user.email.split('@')[0],email:user.email,classCode:genCode(),plan:'medium',maxStudents:30,status:'active',expiresAt:Date.now()+(90*24*60*60*1000)};
      localStorage.setItem('dtwd_teacher',JSON.stringify(currentTeacher));
      showLoading(false);await showDashboard();
    }
...
function tryLocalStorageFallback(){
  ...
  if(saved){try{const p=JSON.parse(saved);if(p.status==='active'&&p.name){currentTeacher=p;showLoading(false);showDashboard();return;}}catch(e){}}
  showLoading(false);showErrorPage('Loading timed out. Please refresh or check your internet connection.');
}

async function updateFromFirestore(uid){
  try{
    const doc=await Promise.race([db.collection('users').doc(uid).get(),new Promise((_,r)=>setTimeout(()=>r(new Error('t')),5000))]);
    if(doc.exists&&doc.data().role==='teacher'){
      const d=doc.data();
      if(d.status==='pending_payment'){window.location.href='pricing.html?plan='+(d.plan||'medium')+'&email='+encodeURIComponent(d.email||'');return;}
      currentTeacher.name=d.name||currentTeacher.name;currentTeacher.classCode=d.classCode||currentTeacher.classCode;
      currentTeacher.plan=d.plan||currentTeacher.plan;currentTeacher.maxStudents={small:15,medium:30,large:60,xl:100}[d.plan]||30;
      currentTeacher.expiresAt=d.expiresAt||currentTeacher.expiresAt;currentTeacher.status=d.status||'active';
      localStorage.setItem('dtwd_teacher',JSON.stringify(currentTeacher));
      const ne=document.getElementById('teacher-name');const ce=document.getElementById('class-code-badge');
      if(ne)ne.textContent=currentTeacher.name;if(ce)ce.textContent='Class Code: '+currentTeacher.classCode;
    }
  }catch(e){}
}
...
function showLoading(on){
  ...
  if(!el&&on){el=document.createElement('div');el.id='loading-overlay';el.style.cssText='position:fixed;inset:0;background:linear-gradient(160deg,#1e1b4b,#5b21b6);display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:9999;color:#fff;font-family:inherit;gap:16px;';el.innerHTML='<div style="font-size:3rem;">🙏</div><div style="font-size:1rem;font-weight:700;">Loading Teacher Dashboard...</div><div style="width:40px;height:40px;border:4px solid rgba(255,255,255,.3);border-top-color:#fff;border-radius:50%;animation:spin 1s linear infinite;"></div>';document.body.appendChild(el);}
...
async function showDashboard(){
  ...
  if(!dash){showErrorPage('Dashboard element not found.');return;}
  dash.style.display='block';dash.classList.add('active');
  const ne=document.getElementById('teacher-name');const ce=document.getElementById('class-code-badge');
  if(ne)ne.textContent=currentTeacher.name||'Teacher';
  if(ce)ce.textContent='Class Code: '+(currentTeacher.classCode||'...');
  await loadStudentsAndSubmissions();updateStats();renderStudentList(allStudents);
}
...
function updateStats(){
  ...
  const s1=document.getElementById('stat-students');const s2=document.getElementById('stat-submissions');const s3=document.getElementById('stat-graded');const s4=document.getElementById('stat-days');
  if(s1)s1.textContent=allStudents.length;if(s2)s2.textContent=allSubmissions.length;if(s3)s3.textContent=allSubmissions.filter(s=>s.status==='graded').length;if(s4)s4.textContent=daysLeft;
}
...
function renderStudentList(students){
  ...
  container.innerHTML='';
  if(!students.length){
    container.innerHTML='<div style="text-align:center;color:#9ca3af;padding:30px 16px;font-size:.88rem;">No students yet.<br><br>Share your class code with students:<br><strong style="font-size:1.1rem;color:#7c3aed;letter-spacing:2px;">'+(currentTeacher.classCode||'—')+'</strong></div>';
    return;
  }
  students.forEach(student=>{
    const totalSubs=student.submissions.length;const gradedSubs=student.submissions.filter(s=>s.status==='graded').length;const pendingSubs=totalSubs-gradedSubs;
    let badges='';
    ...
    const el=document.createElement('div');el.className='student-item'+(currentStudentId===student.id?' active':'');
...
function filterStudents(q){renderStudentList(allStudents.filter(s=>(s.name||'').toLowerCase().includes(q.toLowerCase())||(s.email||'').toLowerCase().includes(q.toLowerCase())));}
...
function selectStudent(studentId,clickedEl){
  ...
  const student=allStudents.find(s=>s.id===studentId);if(!student)return;
  const emptyEl=document.getElementById('detail-empty');const dc=document.getElementById('detail-content');
  if(emptyEl)emptyEl.classList.add('hidden');if(!dc)return;
  dc.classList.remove('hidden');
  const initials=(student.name||student.email||'?').split(' ').map(n=>n[0]).join('').toUpperCase().slice(0,2);
  let subsHtml=student.submissions.length?'<div class="submissions-list">'+student.submissions.map(sub=>renderSubCard(sub,student)).join('')+'</div>':'<div style="text-align:center;color:#9ca3af;padding:20px;font-size:.88rem;">No submissions yet.</div>';
...
function setGradeStyle(style){
  gradeStyle=style;
  const lg=document.getElementById('letter-grades');const mg=document.getElementById('ministry-grades');const bl=document.getElementById('btn-letter');const bm=document.getElementById('btn-ministry');
...
async function submitGrade(){
  ...
  const commentEl=document.getElementById('grade-comment');const comment=commentEl?commentEl.value.trim():'';
  ...
  }catch(e){console.error('Grade save:',e.message);alert('Error saving grade: '+e.message);return;}
