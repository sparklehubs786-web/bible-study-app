  ...
  auth.onAuthStateChanged(async user => {
    ...
    if (savedTeacher) {
      try {
        ...
        // Only trust it if email matches current auth user
        if (parsed.email === user.email) {
          ...
          showLoading(false);
          await showDashboard();
          // Update from Firestore silently in background
    ...
    try {
      ...
      if (!doc.exists) {
        // No profile — create minimal teacher profile
        currentTeacher = {
          uid: user.uid,
      ...
      // ─── CRITICAL FIX: If Firestore says student but user came to dashboard.html,
      //     trust localStorage teacher session (avoid wrong redirects) ───
      if (data.role === 'student') {
        // No teacher session in localStorage either → truly a student
        showLoading(false);
...
async function updateFromFirestore(uid) {
  try {
    ...
    if (doc.exists && doc.data().role === 'teacher') {
      ...
      // Check payment gate in background update too
      if (data.status === 'pending_payment') {
        window.location.href = 'pricing.html?plan=' + (data.plan || 'medium') +
...
function updateStats() {
  ...
  if (statSubmissions) statSubmissions.textContent = allSubmissions.length;
  if (statGraded)      statGraded.textContent      = allSubmissions.filter(s=>s.status==='graded').length;
  if (statDays)        statDays.textContent        = daysLeft;
}
...
function openGradeModal(submissionId, studentId, context) {
  ...
  const ctxEl = document.getElementById('grade-context');
  if (ctxEl) ctxEl.textContent = context||'';
  const commentEl = document.getElementById('grade-comment');
  if (commentEl) commentEl.value = '';
  const selectedEl = document.getElementById('grade-selected');
  ...
  if(sub&&sub.grade){
    ...
    if (commentEl) commentEl.value=sub.comment||'';
    showGradeSelected();
    setGradeStyle(sub.gradeStyle||'letter');
  } else {
    setGradeStyle('letter');
  }
}

function closeGradeModal() {
  const el = document.getElementById('grade-modal');
...
function setGradeStyle(style) {
  gradeStyle=style;
  const lg = document.getElementById('letter-grades');
  const mg = document.getElementById('ministry-grades');
  const bl = document.getElementById('btn-letter');
  const bm = document.getElementById('btn-ministry');
  if (lg) lg.classList.toggle('hidden', style!=='letter');
  if (mg) mg.classList.toggle('hidden', style!=='ministry');
  if (bl) bl.classList.toggle('active', style==='letter');
  if (bm) bm.classList.toggle('active', style==='ministry');
  selectedGrade=null;
  const sel = document.getElementById('grade-selected');
...
function showGradeSelected() {
  ...
  if(selectedGrade&&el){ el.textContent='Selected: '+selectedGrade; el.classList.remove('hidden'); }
}
